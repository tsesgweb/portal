...
if (checarSuporteStorage()) {
  ...
  var $production =
    /^(http(s)?(:\/\/))?(metalic|webhomologa|portalhomologa|www|intranet)?(\.)?(tse|tre\-[a-z]{2})\.(jus|gov)\.br(|\/)$/.test(
      $address
    );
  var $developer =
    /^\/portal\/(?:|index\.php?)$/i.test(window.location.pathname) ||
  ...
  var $storageKey =
    "analytics" + "-" + $subdomain + "-" + $domain.toUpperCase();
  var $time = 100;
  if (!$developer) {
...
function getData(key, time) {
  setTimeout(function () {
    var $getUrl =
      ...
      $setId =
        $getStorage.length >= 1
          ? ($setId = parseInt($getStorage.length) + 1)
          : 1,
      $identifier,
      $equalUrl;
    ...
    function updateItem(id) {
      for (var i = 0; i < $getStorage.length; i++) {
        if (id == $getStorage[i].id) {
          ...
          if ($getStorage[i].title !== titleFormat())
            $getStorage[i].title = titleFormat();
          //Adiciona o contador
          $getStorage[i].access++;
          //Cria o item accessed
          $getStorage[i].accessed = new Date();
          //Cria o item changed e coloca a direfença de dias
          $getStorage[i].changed = differDates(
            $getStorage[i].created,
            $getStorage[i].accessed
          );
        }
      }
      return localStorage.setItem(key, JSON.stringify($getStorage));
...
function titleFormat() {
  ...
  var $regExp = $title.match(
    /(|\s)\—+ Tribunal (Superior|Regional) Eleitoral(.+$|)/gim
  );
